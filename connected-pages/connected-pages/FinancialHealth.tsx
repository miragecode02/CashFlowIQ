...
import { Shield, TrendingUp, AlertTriangle, Sparkles, ArrowUpRight, Loader2 } from "lucide-react";
...
const getHealthScore = (savingsRate: number, hasTransactions: boolean) => {
  if (!hasTransactions) return 50;
  if (savingsRate >= 50) return 90;
  if (savingsRate >= 30) return 75;
  if (savingsRate >= 20) return 60;
  if (savingsRate >= 10) return 45;
  return 30;
};

const getHealthLabel = (score: number) => {
  if (score >= 80) return { label: "Excellent", color: "text-mint" };
  if (score >= 60) return { label: "Good", color: "text-mint" };
  if (score >= 40) return { label: "Fair", color: "text-chart-amber" };
...
const FinancialHealth = () => {
  ...
  const hasData = (summary?.monthly_trends?.length || 0) > 0;
  const healthScore = getHealthScore(savingsRate, hasData);
  const { label, color } = getHealthLabel(healthScore);
  ...
  return (
    <motion.div className="space-y-4 pb-24 px-4 pt-6 max-w-md mx-auto" variants={container} initial="hidden" animate="show">
      ...
      <motion.div variants={item} className="gradient-card p-6 flex flex-col items-center">
        <CircularGauge value={healthScore} size={160} strokeWidth={10} color="hsl(160, 84%, 39%)" />
        <p className={`text-lg font-bold mt-3 ${color}`}>{label}</p>
        <p className="text-xs text-muted-foreground">
          {hasData ? "Based on your real transaction data" : "Add transactions to get a real score"}
        </p>
      </motion.div>
      ...
      <motion.div variants={item} className="glass-card p-4">
        ...
        <Progress value={Math.min(savingsRate, 100)} className="h-2 bg-secondary mt-2" />
      </motion.div>
